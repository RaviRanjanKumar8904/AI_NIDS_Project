...
def load_data():
    ...
    # Destination_Port: Which port is being accessed (e.g., 80 for Web, 22 for SSH)
    # Flow_Duration: How long the connection lasted
    # Total_Fwd_Packets: How many packets were sent
    data = {
    ...
    # Logic: Attacks (Label 1) usually have very high packet counts or very short duration
    # We artificially modify the data so the AI can find a pattern
    
    # Increase packet count for attacks
    df.loc[df['Label'] == 1, 'Total_Fwd_Packets'] += np.random.randint(50, 200, size=df[df['Label'] == 1].shape[0])
    
    # Decrease flow duration for attacks (quick hits)
...
y = df['Label']              # Target (Output: 0 or 1)

# Split data into training (to teach) and testing (to exam)
...
with col_train:
    ...
    if st.button("Train Model Now"):
        with st.spinner("Training Random Forest Classifier..."):
            ...
            st.session_state['model'] = model # Save model to session memory
...
if st.button("Analyze Packet"):
    if 'model' in st.session_state:
        ...
        # We randomize the port for the simulation as it changes frequently
        ...
        if pred[0] == 1:
            ...
            st.write("**Reason:** Suspicious pattern matched (e.g. high packets/low duration).")
        else:
